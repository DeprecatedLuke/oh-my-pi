...
/**
 * Per-{@link StatusLineSession} active-processing meter for the `time_spent`
 ...
 * time. `StatusLineSession.switchSession` (/resume, /move, ACP fork, RPC
 * `switch_session`, extension `switchSession`) mutates the loaded file
 * under the same {@link StatusLineSession} ref, so the WeakMap key alone
...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	 ...
	 */
	setSession(session: TSession, focusedAgentId?: string): void {
	 ...
	 * {@link StatusLineSession} ref (`switchSession` paths: `/resume`, `/move`,
				...
				try {
					const raw =
						(await repository.label(
							AbortSignal.any([request.controller.signal, AbortSignal.timeout(JJ_COMMAND_TIMEOUT_MS)]),
						)) ?? null;
	...
	#lookupPr(activeRepoCache: ActiveRepoCache = this.#resolveActiveRepoCache()): {
		...
		(async () => {
			...
			try {
				...
				const result = await this.host.lookupPullRequest(lookupCwd);
	...
	#getMainSessionTokensPerSecond(): number | null {
		...
		const rate = this.host.calculateTokensPerSecond(this.session.state.messages, this.session.isStreaming);
		if (rate !== null) {
	...
	#getUsageContextKey(session: TSession): string {
		const activeProvider = session.state.model?.provider ?? session.model?.provider;
		const identity = activeProvider ? this.host.activeAccount(session, activeProvider) : undefined;
	...
	#applyUsageRefreshReports(session: TSession, reports: unknown, sequence: number): void {
		...
		const activeProvider = session.state.model?.provider ?? session.model?.provider;
		const activeModelId = session.state.model?.id ?? session.model?.id;
		const activeIdentity = activeProvider ? this.host.activeAccount(session, activeProvider) : undefined;
	...
	#normalizeUsageReports(
		...
		for (const report of reports) {
			...
			for (const limit of limits) {
				...
				if (context.identity && !this.host.limitMatchesActiveAccount(usageReport, usageLimit, context.identity))
					continue;
	...
	getCachedContextBreakdown(): { usedTokens: number; contextWindow: number } {
		...
		const settingsRevision = this.host.getSessionSettingsRevision(this.session);
