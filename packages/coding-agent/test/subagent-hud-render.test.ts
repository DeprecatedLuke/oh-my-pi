/**
 * Contract: the pure subagent HUD renderer lists exactly the running *detached*
 * subagents as `Id: description` rows and yields no output once nothing
 * qualifies. InteractiveMode's retained anchored panel is the live
 * Background Jobs panel, which is fed by AsyncJobManager snapshots.
 */
...
describe("InteractiveMode subagent observer UI sync", () => {
	...
	it("refreshes the retained Background Jobs panel after a burst of progress updates", async () => {
		...
		expect(panel).toContain("[task] BurstAgent0: Burst job 0");
		expect(panel).toContain("[task] BurstAgent5: Burst job 5");
