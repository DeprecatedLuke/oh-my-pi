...
describe("AgentSession auto-snapcompact local-blocker fallback", () => {
	...
	it("does not shake images after automatic context-full compaction", async () => {
		const harness = await createHarness(modelRegistry, {
			activeModel: { provider: "anthropic", id: "claude-sonnet-4-5" },
			// Legacy `compaction.strategy: "context-full"` maps to ["soft"] under
			// the methodOrder settings model (settings migration excludes remote
			// when no endpoint is configured); express it explicitly so the fork
			// auto-shake contract ("no shake after an LLM compaction") holds.
			methodOrder: ["soft"],
		});
		session = harness.session;
		const dropImagesSpy = vi.spyOn(session, "dropImages").mockResolvedValue({ removed: 1 });
