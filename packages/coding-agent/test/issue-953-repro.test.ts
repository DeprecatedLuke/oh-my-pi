...
describe("issue #953 cache status line icons", () => {
	it("renders cache reads and writes with a single database icon", () => {
		...
		expect(cacheRead.content).not.toContain(theme.icon.output);

		expect(cacheWrite.visible).toBe(true);
		expect(cacheWrite.content).toContain(theme.icon.cache);
		expect(cacheWrite.content).not.toContain(theme.icon.input);
