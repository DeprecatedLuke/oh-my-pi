...
describe.skipIf(isWindows)("search with omitted paths", () => {
	...
	it("defaults to the workspace root when paths is an empty JSON array", async () => {
		const tool = new GrepTool(createTestSession(cwd));
...
describe.skipIf(isWindows)("search across unrelated filesystem trees", () => {
	...
	beforeEach(async () => {
		...
		dirB = await fs.mkdtemp(path.join(process.cwd(), ".pi-search-multi-B-"));
	...
	afterEach(async () => {
		await Promise.all([
			dirA ? removeWithRetries(dirA) : Promise.resolve(),
			dirB ? removeWithRetries(dirB) : Promise.resolve(),
			cwd ? removeWithRetries(cwd) : Promise.resolve(),
		]);
	});
