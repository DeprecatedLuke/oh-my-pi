...
export class SessionMaintenance {
	...
	async runAutoCompaction(
		...
		try {
			...
			return await this.#commitAutoCompactionResult({
				...
				runAutoShakeImages: snapcompactResult !== undefined,
			});
		} catch (error) {
			if (autoCompactionSignal.aborted) {
				await this.#emitLifecycleEvent(
	...
	async #commitAutoCompactionResult(args: {
		...
		onCommitted: () => void;
		/** Fork-specific: after a snapcompact rewrite, reuse `/shake images` to drop attached images from the branch. */
		runAutoShakeImages?: boolean;
	}): Promise<CompactionCheckResult> {
		...
		// Fork-specific: after a snapcompact rewrite, reuse `/shake images` to
		// drop attached images from the branch before the post-commit progress
		// checks measure the rebuilt context.
		if (args.runAutoShakeImages) await this.#autoShakeImagesAfterSnapcompact();
