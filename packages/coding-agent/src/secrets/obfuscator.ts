...
import type { AssistantMessage, Context, ImageContent, Message, TextContent, Tool } from "@oh-my-pi/pi-ai";
...
const HASH_LEN = 12;
// Sessions persisted before keyed placeholders used a four-character token
// derived from the entry index. Retain that exact format for display-only
// restoration of historical transcripts.
...
/** Build the pre-keyed, index-derived placeholder used by historical sessions. */
function buildLegacyPlaceholder(index: number): string {
	let value = Bun.hash.xxHash32(String(index), LEGACY_HASH_SEED);
	let tag = "";
	for (let i = 0; i < LEGACY_HASH_LEN; i++) {
		tag += HASH_CHARS[value % HASH_CHARS.length];
		value = Math.floor(value / HASH_CHARS.length);
	}
	return `$$${tag}$$`;
}
...
export class SecretObfuscator {
	...
	/** Historical unkeyed aliases, accepted only on trusted stored-session paths. */
		...
		for (const entry of entries) {
			...
			if (mode === "obfuscate") {
				if (entry.content.length < MIN_OBFUSCATE_SECRET_LEN) {
					// Tone down short plain secret obfuscation to avoid false matches on small words like "$$ENVSTATE_A6O2XH3Q1VIF:L$$".
					continue;
				}
		...
		for (const entry of this.#regexEntries) {
			...
			for (const match of matches) {
				if (entry.mode === "replace") {
					...
					if (match.straddledGeneratedPlaceholder && match.scanMatchLength < MIN_OBFUSCATE_SECRET_LEN) {
						// A short deliberate regex match is still a secret and must be
						// masked. The threshold applies only when the match straddles an
						// existing placeholder (from this or a prior pass): rewriting that
						// widened span would absorb surrounding literals and break
						// round-trip/fixed-point behavior.
						continue;
					}
	...
	#collectRegexMatches(
		...
		straddledGeneratedPlaceholder: boolean;
		...
		for (;;) {
			...
			const straddledGeneratedPlaceholder = mapped.partialPlaceholderCut;
...
/** Redact tool descriptions and wire schemas without walking live schema instances. */
...
/**
 * Redact outbound provider context. Conversation messages are selectively
 * redacted; system prompts and tool schemas are deep-walked because either
 * can carry secrets sourced from the workspace, memory, or extensions.
 */
export function obfuscateProviderContext(obfuscator: SecretObfuscator | undefined, context: Context): Context {
	...
	const systemPrompt = obfuscator.obfuscateObject(context.systemPrompt);
	const messages = obfuscateMessages(obfuscator, context.messages);
	const tools = obfuscateProviderTools(obfuscator, context.tools);
...
function deepWalkStrings<T>(obj: T, transform: (s: string) => string): T {
	...
	if (obj !== null && typeof obj === "object" && isPlainRecord(obj)) {
		...
		// Binary provider payloads are opaque: replacing a coincidental secret
		// substring in base64 corrupts the image/ciphertext. This also covers
		// snapcompact frames, which carry `mimeType` without a `type` field.
		const isBinaryCarrier =
			record.type === "image" ||
			record.type === "base64" ||
			typeof record.mimeType === "string" ||
			typeof record.media_type === "string";
		const skipData = (isBinaryCarrier || record.type === "redactedThinking") && typeof record.data === "string";
		let changed = false;
		const result: Record<string, unknown> = {};
		for (const key of Object.keys(record)) {
			const value = record[key];
			if (skipData && key === "data") {
				result[key] = value;
				continue;
			}
