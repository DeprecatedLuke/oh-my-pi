...
- **`wait`** (or `await: true`): blocks until matching message, timeout, or steering interrupt. Parent IRC interrupts at steering priority. NEVER use for job waiting; job results auto-deliver; sole blocker means end turn immediately without prose or tool calls.
