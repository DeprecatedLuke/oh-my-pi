...
3. **Dispatch phase.** Launch all parallel `task` subagents in one message, then end the turn. Their results return automatically in a follow-up before the next phase.
