...
{{/if}}
- **Subagent type:** `{{toolRefs.task}}` accepts an `agent` parameter to select the subagent type. Pick the type that matches the work: `agent: "research"` for web-search-based investigation of unfamiliar technologies/APIs (no file access, returns recommendations); `agent: "explore"` for read-only codebase investigation; omit `agent` for the default worker (edits + implementation).
- **Waiting for results:** When you have spawned subagents and have nothing else to do, end your turn. Do NOT spin, poll, or call tools just to wait — results deliver automatically as a follow-up turn when each subagent finishes. Ending your turn is the correct action, not a yield of incomplete work.
{{/has}}
{{/if}}
