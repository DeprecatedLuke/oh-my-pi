...
interface Pending {
	...
	hadColon: boolean;
	/** True when this pending replace came from a bare `N.=M:` recovery header. */
	implicitBareRange: boolean;
	/**
...
export class Executor {
	...
	feed(token: Token): void {
		...
		switch (token.kind) {
			...
			case "op-block": {
				...
				const snapshotRow = this.#snapshotRows.length === 1 ? this.#snapshotRows[0] : undefined;
				const sameSnapshotRange =
					snapshotRow !== undefined &&
					target.kind === "replace" &&
					target.range.start.line === snapshotRow.line &&
					target.range.end.line === snapshotRow.line;
				if (sameSnapshotRange) {
					this.#snapshotRows = [];
				...
				const pendingBareRange =
					this.#pending?.implicitBareRange === true && this.#pending.payloads.length === 0
						? this.#pending.target
						: undefined;
				if (pendingBareRange !== undefined) {
					const sameReplaceRange =
						pendingBareRange.kind === "replace" &&
						target.kind === "replace" &&
						pendingBareRange.range.start.line === target.range.start.line &&
						pendingBareRange.range.end.line === target.range.end.line;
					if (!sameReplaceRange) {
						throw new Error(
							`line ${token.lineNum}: bare range header cannot be followed by a different hunk. ` +
								"Prefix the range with `PUT`/`SWAP` and keep one hunk per range.",
						);
					}
				}
				this.#flushPending();
	...
	#flushSnapshotRows(): void {
		...
		if (rows.length === 1) {
			const row = rows[0]!;
			throw new Error(
				`line ${row.lineNum}: line ${row.line} content was pasted without a hunk header. ` +
					`Add \`SWAP ${row.line}.=${row.line}:\` on the line above, then the body rows below.`,
			);
		}
		for (const row of rows) {
			const range = { start: { line: row.line }, end: { line: row.line } };
			validateRange(range, row.lineNum, "replace");
			this.#pushInsert({ kind: "before_anchor", anchor: { line: row.line } }, row.text, row.lineNum, "replacement");
			this.#pushDeleteRange(range, row.lineNum);
		}
	...
	#handleLiteralPayload(text: string, lineNum: number): void {
		const pending = this.#pending;
		if (!pending) this.#flushSnapshotRows();
		if (!pending) {
	...
	#handleRaw(text: string, lineNum: number): void {
		...
		if (this.#pending === undefined && snapshotRow !== null) {
			validateRange({ start: { line: snapshotRow.line }, end: { line: snapshotRow.line } }, lineNum, "replace");
			this.#snapshotRows.push({ ...snapshotRow, lineNum });
			return;
		}
		if (this.#pending === undefined) this.#flushSnapshotRows();
