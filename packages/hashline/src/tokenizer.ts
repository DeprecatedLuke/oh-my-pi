 ...
 * `:` (e.g. `SWAP 2.=3.:`, `INS.POST 2.:`). A `.` is never valid at this
 * position, so skip it when it precedes an optional `:` or end-of-line.
 */
...
function consumeOptionalColon(line: string, index: number, end: number): ColonScan {
	let cursor = skipStrayDot(line, skipWhitespace(line, index, end), end);
	if (cursor < end && line.charCodeAt(cursor) === CHAR_COLON) {
		...
		return { nextIndex: cursor, hadColon: true };
	}
	if (cursor < end && line.charCodeAt(cursor) === CHAR_EQUALS) {
		const afterEquals = skipWhitespace(line, cursor + 1, end);
		if (afterEquals < end && line.charCodeAt(afterEquals) === CHAR_COLON) {
			return { nextIndex: skipWhitespace(line, afterEquals + 1, end), hadColon: true };
		}
	}
...
function scanHunkAnchor(line: string, start: number, end: number): TargetScan | null {
	...
	const swapEnd = scanKeyword(line, cursor, end, "SWAP");
	if (swapEnd !== null) {
		const target = scanPutTarget(line, swapEnd, end);
		if (target === null || target.hadColon) return target;
		return { ...target, hadColon: true };
	}
	const delEnd = scanKeyword(line, cursor, end, "DEL");
	if (delEnd !== null) return scanCutTarget(line, delEnd, end);
	const insertBeforeEnd = scanKeyword(line, cursor, end, "INS.PRE");
	if (insertBeforeEnd !== null) {
		const anchor = scanLineNumber(line, skipWhitespace(line, insertBeforeEnd, end), end);
		if (anchor === null) return null;
		const target = finishTargetScan(line, anchor.nextIndex, end, {
			kind: "insert_before",
			anchor: { line: anchor.line },
		});
		return target.hadColon ? target : { ...target, hadColon: true };
	}
	const insertAfterEnd = scanKeyword(line, cursor, end, "INS.POST");
	if (insertAfterEnd !== null) {
		const anchor = scanLineNumber(line, skipWhitespace(line, insertAfterEnd, end), end);
		if (anchor === null) return null;
		const target = finishTargetScan(line, anchor.nextIndex, end, {
			kind: "insert_after",
			anchor: { line: anchor.line },
	...
	const insertHeadEnd = scanKeyword(line, cursor, end, "INS.HEAD");
	if (insertHeadEnd !== null) {
		const target = finishTargetScan(line, insertHeadEnd, end, { kind: "bof" });
		return target.hadColon ? target : { ...target, hadColon: true };
	}
	const insertTailEnd = scanKeyword(line, cursor, end, "INS.TAIL");
	if (insertTailEnd !== null) {
		const target = finishTargetScan(line, insertTailEnd, end, { kind: "eof" });
		return target.hadColon ? target : { ...target, hadColon: true };
	}
...
function classifyLine(line: string, lineNum: number): Token {
	...
	const isHunkLead =
		...
		line.startsWith("INS.PRE", lead) ||
		line.startsWith("INS.POST", lead) ||
		line.startsWith("INS.HEAD", lead) ||
		line.startsWith("INS.TAIL", lead);
	if (isHunkLead) {
