...
describe("hashline core — verb header forms", () => {
	...
	// literal row by spec, so it was inserted into the Rust file as text. The
	// `+` prefix is what makes it literal; omit it when the operation is intended
	// to execute.
	it("keeps a body row that resembles a hunk header literal", () => {
	...
	it("recovers a bare range body before a subsequent hunk", () => {
...
describe("hashline body contracts", () => {
	...
	it("recovers unified-diff old rows when the replacement range already deletes them", () => {
