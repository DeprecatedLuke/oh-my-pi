...
- Fixed Codex streams that close before a terminal completion event being classified as transient, allowing the existing auto-retry flow to continue the conversation.
...
- Added ClinePass as a native `clinepass` provider with an API-key login flow (`CLINE_API_KEY`, `sk_…` keys). ClinePass is Cline's flat-rate subscription gateway at `https://api.cline.bot/api/v1` that re-hosts open coding models (GLM, DeepSeek, Kimi, Qwen, MiMo, MiniMax) behind one OpenAI-compatible endpoint; its keys reject `/v1/models`, so login validation pings chat completions against the default model directly. Added a `clinepass` `wireModelIdMode` that translates the friendly bare catalog id (`glm-5.2`) to the `cline-pass/glm-5.2` wire form at request time. The compat is gated on the `clinepass` provider id (not the shared `api.cline.bot` host, which also serves generic passthrough model ids), forwards `reasoning_effort` verbatim across `minimal`..`xhigh` (the gateway 400s only on `max`), and reads streamed chain-of-thought from `delta.reasoning`. Login validation probes chat completions with the runtime `max_completion_tokens` field and a real token budget (the OpenAI-compatible validator gained optional `maxTokensField`/`maxTokens` overrides), so a valid key on a ClinePass reasoning model is not rejected by a 1-token probe that returns empty content.
- Added ClinePass as a native `clinepass` provider with an API-key login flow (`CLINE_API_KEY`, `sk_…` keys). ClinePass is Cline's flat-rate subscription gateway at `https://api.cline.bot/api/v1` that re-hosts open coding models (GLM, DeepSeek, Kimi, Qwen, MiMo, MiniMax) behind one OpenAI-compatible endpoint; its keys reject `/v1/models`, so login validation pings chat completions against the default model directly. Added a `clinepass` `wireModelIdMode` that translates the friendly bare catalog id (`glm-5.2`) to the `cline-pass/glm-5.2` wire form at request time. The compat is gated on the `clinepass` provider id (not the shared `api.cline.bot` host, which also serves generic passthrough model ids), forwards `reasoning_effort` verbatim across `minimal`..`xhigh` (the gateway 400s only on `max`), and reads streamed chain-of-thought from `delta.reasoning`. Login validation probes chat completions with the runtime `max_completion_tokens` field and a real token budget (the OpenAI-compatible validator gained optional `maxTokensField`/`maxTokens` overrides), so a valid key on a ClinePass reasoning model is not rejected by a 1-token probe that returns empty content. ClinePass login also supports **WorkOS OAuth auto-login**: when the user is signed in via the Cline CLI, its credentials are reused (no API key needed) and the short-lived access token is refreshed via `POST /api/v1/auth/refresh`, falling back to the manual API-key paste otherwise. (WorkOS credential/refresh contract adapted from [jellydn/pi-clinepass-provider](https://github.com/jellydn/pi-clinepass-provider), MIT.)
