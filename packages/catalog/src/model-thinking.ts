...
function getModelDefinedEfforts<TApi extends Api>(
	...
	// ClinePass uses the standard OpenAI ladder through `xhigh` and rejects
	// `max`; its MiMo SKUs retain the family-wide low/medium/high cap.
	if (spec.provider === "clinepass") {
		return isMimoModelIdOrName(spec.id) || isMimoModelIdOrName(spec.name ?? "")
