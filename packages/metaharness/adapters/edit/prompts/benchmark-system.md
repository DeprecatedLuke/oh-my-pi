...
- If an edit was applied (no error, tag advanced), it is correct — proceed to the next edit or stop. Do not re-read to verify. Warnings (including auto-repair) are informational — the result already shows the final state. Re-reading in a loop is worse than stopping.
